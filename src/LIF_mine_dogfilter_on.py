...
if __name__ == '__main__':
    ...
    log_path = f'../log/{now}/'
    fig_path = f'../fig/{now}/'
    dataset_path = '../dataset/ESC-50-master/audio/'
    ...
    def CosSim_w_a1():
        ...
        # 対角線を除いた平均類似度の計算
        mean_cossim_1 = np.mean(cossim_1[np.triu_indices_from(cossim_1, k=1)])

        return cossim_1, mean_cossim_1
    ...
    def CosSim_w_a2():
        ...
        # 対角線を除いた平均類似度の計算
        mean_cossim_2 = np.mean(cossim_2[np.triu_indices_from(cossim_2, k=1)])

        return cossim_2, mean_cossim_2

    def Correla_w_a2():
        x = np.corrcoef(w_a2.T)
    ...
    def CosSim_w_a3():
        ...
        # print(cossim_2.shape)
        # 対角線を除いた平均類似度の計算
        mean_cossim_3 = np.mean(cossim_3[np.triu_indices_from(cossim_3, k=1)])
        return cossim_3, mean_cossim_3
    ...
    while True:
        ...
        for idx, fpath in enumerate(file_list):  # file_listの各要素に対してインデックス番号とファイルパスを取得
            ...
            for t in range(normalized_dog_image.shape[1]):
                ...
                # 画像描画
                if t == normalized_dog_image.shape[1] - 1:
                    if draw:
                        ...
                        cossim_1, mean_cossim_1 = CosSim_w_a1()
                        cossim_2, mean_cossim_2 = CosSim_w_a2()
                        cossim_3, mean_cossim_3 = CosSim_w_a3()
                    ...
                    if key == 27:
                        ...
                        cv2.imwrite(fig_path + 'CosSim_w_a1' + now_2 + '.png', ImgConvert(CosSim_w_a1()[0]))
                        cv2.imwrite(fig_path + 'Correlation_w_a1' + now_2 + '.png', ImgConvert(Correla_w_a1()))
                        cv2.imwrite(fig_path + 'CosSim_w_a2' + now_2 + '.png', ImgConvert(CosSim_w_a2()[0]))
                        cv2.imwrite(fig_path + 'Correlation_w_a2' + now_2 + '.png', ImgConvert(Correla_w_a2()))
                        cv2.imwrite(fig_path + 'CosSim_w_a3' + now_2 + '.png', ImgConvert(CosSim_w_a3()[0]))
                        cv2.imwrite(fig_path + 'Correlation_w_a3' + now_2 + '.png', ImgConvert(Correla_w_a3()))
                        cv2.imwrite(fig_path + 'log_a1_' + now_2 + '.png', ImgConvert(log_a1))
                        cv2.imwrite(fig_path + 'log_a2_' + now_2 + '.png', ImgConvert(log_a2))
                        cv2.imwrite(fig_path + 'log_a3_' + now_2 + '.png', ImgConvert(log_a3))
                        cv2.imwrite(fig_path + 'w_a1_' + now_2 + '.png', ImgConvert(w_a1 / w_a1.max()))
                        cv2.imwrite(fig_path + 'w_a2_' + now_2 + '.png', ImgConvert(w_a2 / w_a2.max()))
                        cv2.imwrite(fig_path + 'w_a3_' + now_2 + '.png', ImgConvert(w_a3 / w_a3.max()))
                        cv2.imwrite(fig_path + 'w_a2_img' + now_2 + '.png', ImgConvert(w_a2_img / w_a2_img.max()))
                        cv2.imwrite(fig_path + 'w_a3_img' + now_2 + '.png', ImgConvert(w_a3_img / w_a3_img.max()))
                        cv2.imwrite(fig_path + 'csp_and_Dogfiltered_lcspr_img' + now_2 + '.png', ImgConvert(np.vstack((lcspr / lcspr.max(), line, normalized_dog_image/255))))
                        cv2.imwrite(fig_path + 'log_a1_img' + now_2 + '.png', ImgConvert(np.vstack((lcspr / lcspr.max(), line, log_hc, line, a1_simg.T / a1_simg.max(), line, a2_simg.T / a2_simg.max(), line, a3_simg.T / a3_simg.max()))))

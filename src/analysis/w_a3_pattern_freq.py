...
def main(log_time: str):
    ...
    npy_file_path = os.path.join(log_folder_, "w_a3.npy")

    # w_a1
    w_a3: np.ndarray = np.load(npy_file_path)
    ...
    def column_patterrns():
        column_patterns = [tuple(w_a3[:, i]) for i in range(w_a3.shape[1])]
    ...
    plt.title('Histogram of Pattern Frequencies_wa_3', size=20)

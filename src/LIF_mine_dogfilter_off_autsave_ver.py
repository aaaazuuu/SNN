...
if __name__ == '__main__':
    ...
    a1 = LIFNeuron(N=n_a1, adapt=True, lateral=lateral) # WTA機構のお話はしている（即抑制→発火したら周りを抑制していくよ　lateralでやっている）
    ...
    while True:
        ...
        for idx, fpath in enumerate(file_list):  # file_listの各要素に対してインデックス番号とファイルパスを取得
            ...
            # ニューラルネットワークの各層の更新
            for t in range(lcspr.shape[1]):
                # update input
                x = lcspr[:, t] / lcspr.max()  # 入力データの配列で、時刻tにおけるすべての入力を取得　データの最大値で割ることでxを正規化
                ...
                # 画像描画
                save_interval = 2000 # 何ステップ目で保存するかのパラメータ
                ...
                if t == lcspr.shape[1] - 1:
                    ...
                    if draw:
                        ...
                        cv2.imshow('log_a1_img', np.vstack((lcspr / lcspr.max(), line, log_hc, line,  a1_simg.T / a1_simg.max(), line, a2_simg.T / a2_simg.max(), line, a3_simg.T / a3_simg.max())))
